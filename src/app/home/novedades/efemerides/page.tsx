...
export default function EfemeridesPage() {
  ...
  // leer ?q= de la URL (lo actualiza CategorySearch)
  ...
  // Filtrar por título (q) + ordenar
  useEffect(() => {
    // 1) Filtrar por título si hay q
    const filtered = posts.filter((post) =>
      q ? post.titulo.toLowerCase().includes(q) : true
    );

    // 2) Ordenar igual que antes, pero sobre los filtrados
    const sorted = [...filtered].sort((a, b) => {
      switch (sortOption) {
        case "date-desc": {
          const dateA = a.fechaEfemeride
            ? new Date(a.fechaEfemeride).getTime()
            : new Date(a.createdAt).getTime();
          const dateB = b.fechaEfemeride
            ? new Date(b.fechaEfemeride).getTime()
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-3 text-neutral-500 py-2 border border-neutral-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
      ...
      <CategorySearch placeholder="Buscar efemérides por título..." />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sortedPosts.map((post) => {
          const firstImage = post.blocks.find(
            (block) => block.imageUrls && block.imageUrls.length > 0
          )?.imageUrls?.[0];
